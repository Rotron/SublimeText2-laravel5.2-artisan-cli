...
self.window.run_command("exec", {
	"cmd": [
		"php /usr/local/bin/composer create-project --prefer-dist laravel/laravel " +
		cmd
	],
	"shell": True,
	"working_dir": folder
...
class ArtisanMakeCommand(sublime_plugin
		.WindowCommand):
	def run(self):
	self.window.show_input_panel(
		"[auth|console|controller|event|job|listener|middleware|migration|model|policy|provider|request|seeder|test] [args] [Es. controller name --resource]",
		"", self.on_done, None, None)
def on_done(self, generate_command):
	try:
	folder = self.window.folders()[0]
...
class ArtisanMigrateCommand(
	...
	def run(self):
	self.window.show_input_panel(
		"[install|refresh|reset|rollback|status] [args]",
		"", self.on_done, None, None)
def on_done(self, generate_command):
	try:
	folder = self.window.folders()[0]
...
self.window.run_command("exec", {
	...
	pass
class ArtisanQueueCommand(
		sublime_plugin.WindowCommand):
	def run(self):
	self.window.show_input_panel(
		"[failed|failed-table|flush|forget|listen|restart|retry|table|work] [args]",
		"", self.on_done, None, None)
def on_done(self, generate_command):
	try:
	folder = self.window.folders()[0]
...
self.window.run_command("exec", {
	...
	pass
class ArtisanRouteCommand(
		sublime_plugin.WindowCommand):
	def run(self):
	self.window.show_input_panel(
		"[cache|registration|clear|list] [args]",
		"", self.on_done, None, None)
def on_done(self, generate_command):
	try:
	folder = self.window.folders()[0]
